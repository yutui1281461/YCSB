...
public class Neo4jClient extends DB {
  ...
  private final String nodeIndexName = "nodes";
  private final String relationshipIndexName = "relationships";
      ...
      if (graphDbInstance == null) {
        ...
        if (useIndex) {
          try (Transaction transaction = graphDbInstance.beginTx()) {
            ...
            if (!index.existsForNodes(nodeIndexName)) {
              nodeIndex = index.forNodes(nodeIndexName);
            }
            if (index.existsForRelationships(relationshipIndexName)) {
              relationshipIndex = index.forRelationships(relationshipIndexName);
            }
            transaction.success();
          }
        }
